...
A t-test, also known as Student's t-test, is a statistical tool used for hypothesis testing to evaluate the means of one or two populations. It helps in determining if there are significant differences between groups.
...
- **Two-sample t-test (Independent):** Used to compare the means of two different groups.
- **Paired t-test (Dependent):** Used to compare means within the same group at different times or under different conditions.

## Usage of t-Tests

1. **Define the Hypothesis:** Start by stating the null and alternative hypotheses based on the research question.
2. **Calculate the Test Statistic:** Use sample data to compute a test statistic which is then compared against a critical value from the t-distribution.
3. **Decision Making:** Based on the comparison, decide to either reject or fail to reject the null hypothesis.

## When Not to Use t-Tests

If you're dealing with more than two groups, a t-test might not be appropriate. Instead, consider methods like ANOVA for multiple comparisons.
...
- Approximate normality of the data distribution
- Independence of samples (for two-sample t-tests)



## One-tailed vs. Two-tailed Tests

- **One-tailed test:** Used when the hypothesis predicts a direction of the effect.
- **Two-tailed test:** Used when the hypothesis does not predict a direction.
